...
def solvep2(path):
    ...
    #calc the result
    #return calcMagintude(addTuple(tupleList[6],tupleList[0]))
    i = 0
    _highscore = 0
    _result = 0
    while i < len(tupleList):
        ...
        while j < len(tupleList):
            _result = calcMagintude(addTuple(deepcopy(tupleList[i]),deepcopy(tupleList[j])))
            if _result >_highscore:
                _highscore = _result
            _result = calcMagintude(addTuple(deepcopy(tupleList[j]),deepcopy(tupleList[i])))
            if _result >_highscore:
                _highscore = _result
            j += 1
        i += 1
